...
language:
- es
pretty_name: Spanish Cadastre INSPIRE
tags:
- geospatial
- cadastre
- buildings
...
def stage(config: Config) -> Path:
    ...
    manifest = {
        ...
        "excluded": ["original GML", "source ZIP archives", "catalog caches"],
    }
...
def upload(config: Config, repo_id: str) -> None:
    ...
    subprocess.run(
        ["hf", "upload", repo_id, str(staging), ".", "--type", "dataset", "--commit-message", "Publish Spanish Cadastre INSPIRE snapshot"],
        check=True,
    )
