...
[ordered]@{snapshot_date=$cfg.snapshot_date; published_date=$cfg.published_date; bytes=(Get-Item $target).Length; md5=$cfg.md5; sha1=$cfg.sha1; completed_at=(Get-Date).ToUniversalTime().ToString('o')} | ConvertTo-Json | Set-Content (Join-Path $raw 'manifest.json') -Encoding utf8
